...
Object.defineProperty(Element.prototype, 'attachShadow', {
  value: Element.prototype.attachShadow,
  enumerable: true,
  configurable: false,
  writable: false,
});
...
const formData = (o) =>
  Object.entries(o).reduce((d, [k, v]) => d.append(k, v) || d, new FormData());
...
const fmtTime = (date) => {
  ...
  return Math.abs(diffMin) < 60
    ? relFmt.format(diffMin, 'minute')
    : Math.abs(diffHour) < 24
    ? relFmt.format(diffHour, 'hour')
    : Math.abs(diffDay) < 7
...
#msgs {
  ...
  #autoInsert = ({ fieldName, ...data }) => {
    if (fieldName in Chat.autoInsert) {
      ...
      this.dispatchEvent(
        new CustomEvent(this.channel, { detail: msg, bubbles: true })
      );
